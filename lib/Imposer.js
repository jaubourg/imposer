...
class Imposer extends NativePromise {
    ...
    static create( creator ) {
        ...
        return new Imposer( ( resolve, _, notify ) => resolve( creator( notify ) ) );
    }

    static from( arg ) {
        checkStaticContext( `from`, this, Imposer );
        return new Imposer( ( resolve, _, { each } ) =>
            // eslint-disable-next-line no-void
            resolve( Imposer.resolve( arg ).then( items => void each( items ), null, () => undefined ) )
        );
    }

    static fromInOrder( arg ) {
        checkStaticContext( `fromInOrder`, this, Imposer );
        return new Imposer( ( resolve, _, { "queue": { each } } ) =>
            // eslint-disable-next-line no-void
            resolve( Imposer.resolve( arg ).then( items => void each( items ), null, () => undefined ) )
        );
    }
    ...
    static of( ...args ) {
        ...
        return Imposer.from( args );
    }

    static ofInOrder( ...args ) {
        checkStaticContext( `ofInOrder`, this, Imposer );
        return Imposer.fromInOrder( args );
    }
