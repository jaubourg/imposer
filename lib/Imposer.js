...
class Imposer extends NativePromise {
    ...
    constructor( executor ) {
        ...
        const internalNotify = state.beforeDone( track( doNotify, doNotify ) );
        const notify = toUser( Imposer, state.beforeResolveDone( internalNotify ) );
        notify.each = toUser( Imposer, state.beforeResolveDone( track( collection => {
            if ( !collection || !collection[ Symbol.iterator ] ) {
                throw new TypeError( `Imposer notify.each(): iterable expected` );
            }
            return NativePromise.all( Array.from( collection ).map( internalNotify ) );
        } ) ) );
        const internalQNotify = state.beforeDone( queueAndTrack( doNotify, doNotify ) );
        notify.queue = toUser( Imposer, state.beforeResolveDone( internalQNotify ) );
        notify.queue.each = toUser( Imposer, state.beforeResolveDone( queueAndTrack( collection => {
            if ( !collection || !collection[ Symbol.iterator ] ) {
                throw new TypeError( `Imposer notify.queue.each(): iterable expected` );
            }
    ...
    static of( item, inOrder = false ) {
        checkStaticContext( `of`, this, Imposer );
